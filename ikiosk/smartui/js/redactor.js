...
(function($){
	...
	var Redactor = function(element, options)
	{
		...
		this.opts = $.extend({
			...
			css: false, // url
			basehref: false, //url
			jqui: false, //jQuery Resources for Resize & Draggable
	...
	Redactor.prototype = {
		...
		build: function(mobile, whendone)
		{
			if (mobile !== true)
			{
				...
				function initFrame()
				{
					this.$editor = this.$content.contents().find("body").attr('contenteditable', true).attr('dir', this.opts.direction).attr('id', 'redactor-iframe');
					...
					if (this.opts.jqui !== false)
					{
						
						
					}
					
					if (this.opts.basehref !== false)
					{
						this.$content.contents().find('head').append('<base href="' + this.opts.basehref + '" />');
					}
				...
				if (this.$el.get(0).tagName === 'TEXTAREA')
				{
					...
					{
						var me = this;
						this.$content = $('<iframe style="width: 100%;" frameborder="0" name="redactor-frame"></iframe>').load(function()
						{
							initFrame.call(me);
						});
					}
				...
				if (this.$editor)
				{
					this.$editor.addClass('redactor_editor').attr('contenteditable', true).attr('dir', this.opts.direction).attr('id', 'redactorEditor');
				}
		...
		observeScroll: function()
		{
			...
			if (scrolltop > boxtop)
			{
				...
				this.fixed = true;
				this.$toolbar.css({ position: 'fixed', width: width, zIndex: 500000, top: this.opts.fixedTop + 'px', left: left });
			}
			else
			{
				this.fixed = false;
		...
		resizeImage: function(resize)
		{
			...
			$(resize).unbind('hover mousedown mouseup click mousemove');
